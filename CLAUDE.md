...
3. **Actions are integer INDICES, not scaling factors.** Policy outputs `a_j ∈ {0,…,m_j-1}` per slot; a decode rule (`sf_from(idx, max_sf, levels, step) = max_sf - step*(levels-1-idx)` for SF slots; the `K_LEVELS` table for K slots) maps each index to the actual `scaling_factor` or truncation `k`. **Per-kind step (2026-06-04):** non-rescale SF kinds (F/W/M/S) use `step=2` at their original level counts (unchanged); **rescale (R) slots use `step=1` with `LEVELS_R=15`** — a deep downward sweep (idx0=None, idx1..14 = max-13..max) so the fusion-map builder probes the SF range where the optimizer actually fuses (the old 3-value sweep only reached fusion 0/1). `_snap_to_table` floors any decoded SF below the noise table (min SF=10), so low-`max_sf` rescales get duplicate SF=10 entries the builder dedups. Baseline (max idx → max_sf) is unchanged. Always log both `action_index` AND `decoded_value`. K decoding is **non-monotonic** in some checkpoints — find all-max via the largest `k` value, not the largest index.
