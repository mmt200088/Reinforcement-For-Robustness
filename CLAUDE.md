...
3. **Actions are integer INDICES, not scaling factors.** Policy outputs `a_j ∈ {0,…,m_j-1}` per slot; a decode rule (`sf_from(idx, max_sf, levels)` for SF slots; the `K_LEVELS` table for K slots) maps each index to the actual `scaling_factor` or truncation `k`. **2026-06-11 decode — UNIFORM step-1 sweep, 15 levels, no floor (user spec; supersedes the 2026-06-04 hybrid 2/1 sweep):** `LEVELS_F=W=MS=R=15`; level value = baseline − dist, i.e. the full integer range `[baseline-14, baseline]` (e.g. baseline 30 → `30,29,28,…,16` — same reach as the old hybrid at integer resolution). **The anchor `max_sf` is the slot's BASELINE SF** (calibrated `MaxSFsTable` from static_skeletons), **not** the noise-table max (46): you sweep *down from the baseline SF*. There is NO extra floor; `_snap_to_table` still bounds below-table values at the table min (SF=10) — a low-baseline slot gets duplicate SF=10 levels, and the fusion builder skips those pre-enumeration via `action_space.distinct_sf_level_indices` (result-equivalent: a duplicate-value level decodes to the identical cfg; the lowest index per value is kept = the lex-min representative the post-eval signature dedup would keep). Rescale (R) keeps idx0=None (idx 1..14 sweep `baseline-13..baseline`). Grid history: hybrid 2/1 ×10 (2026-06-04) → uniform-2/floor-12 (2026-06-10, reverted same day) → **uniform-1 ×15 (2026-06-11, current)**. **Changing this grid invalidates ALL committed fusion maps (they store action indices) — rebuild via SERVER_COMMAND before any fusion run** (the dormant `block5_n0.json` stays stale until degree-0 is ever re-enabled). **Noise variance uses the N=16384 column for every slot** (`_block_default_N` forced to 16384; N=8192 retired). Baseline (max idx → baseline SF) is unchanged → `option0==baseline` holds. **Fusion-map builder pinning uses the `(fusion_count, total_bits)` criterion (NOT fusion-only).** The 2026-06-04 "relax to fusion_count only" change was a bug, reverted: committed ground-truth maps show every block's fusion>0 option is reached by lowering 2–4 non-rescale ENCODE SFs *jointly* while all rescales stay at baseline (e.g. block2 fc=1 = `inv_std_fresh 28→20, gamma 20→16, wk 22→16`); no single encode moves fusion alone, so a fusion-only solo-probe pins them all and the map collapses to `fusion={0}`. `total_bits` here is a build-time over-enumeration PROXY (it is NOT in the reward) that keeps every fusion-driving encode in the joint cartesian enumeration. Build acceleration (result-equivalent, layered): (a) distinct-value level pre-dedup + `action_vector_to_cfgs(..., only=(L,B))` single-block decode (2026-06-10); (b) **direct-replan fast path** (2026-06-11, `blb_stage2_rl/fusion_enum_fast.py`, builder default `--enum-path fast`): one `ReplanSession.replan` is ~0.06 ms but the golden cfg pipeline paid ~10× in intermediates (cfg construction, the bridge's per-call cache-key + `deepcopy` store — the enumeration's combos are all distinct so that cache can never hit — output→cfg write-back, cfg introspection), so the hot loop patches `(t_new, delta_overrides)` directly and assembles installed points from the raw output: ~0.099 ms/combo measured, torch-free worker processes, contiguous rank-range sharding via mixed-radix unranking (no modulo skip-spin). Correctness gates are mandatory: the template is golden-DERIVED (per-slot two-level probes must be an exact identity map of the decoded SF; Q/K mirrors discovered through the real `sync_block*` fns), `verify_template` cross-checks baseline+corner+N random combos golden-vs-fast per build (abort on mismatch), and `--enum-path both` runs BOTH paths fully and requires identical option lists (used for small block-types each rebuild). GPU does NOT apply: replan is pure-Python integer modulus-chain planning; a CUDA version would be a second implementation of the cost source of truth (forbidden). `--max-enum-combos N` stays as the over-budget degeneracy-probe guard (full builds use 0). Always log both `action_index` AND `decoded_value`. K decoding is **non-monotonic** in some checkpoints — find all-max via the largest `k` value, not the largest index.
   **Current truncation-K contract (2026-07-27):** `K_LEVELS=(8, 9, 11, 13, 10, 12, 6, 7)`. Historical indices `0..5` retain their exact meanings; index `3` remains the all-max/baseline `K=13`; indices `6`/`7` are `K=6`/`K=7`. The canonical layerwise policy remains six slots per layer (one Block4 fusion slot plus five K slots), with each K slot now 8-way. Cost/reward normalization uses the full `13-6` interval. Six-category checkpoints are incompatible and require a fresh run. Fusion maps do not need rebuilding for this change because the old indices are stable and K is independent of the fusion-map option.
...
6. **Two episode formulations.** The legacy single-shot path (`BLBStage2Env`, `BLBStage2Policy`, `runner.BLBStage2RLRunner`) treats every `env.step(action_vec)` as a horizon-1 episode that emits the full config in one go. The canonical layerwise path exposes one six-slot action per layer. Every layer has five active K choices; layer-0 Block1 K is installed as truncation-only (`noise_enabled=False`) even though layer-0 Block1 still has no SF/fusion replan request. The retired per-block sequential schedule has 47 optimizer-oriented decision steps for BERT-base, while terminal RO materialization has 59 SF/fusion requests; neither number is the model-side K count, which is 60.
7. **Reward is hard-priority, not weighted-sum.** `invalid/accuracy → stability → cost`. Cost reward must never offset an accuracy or stability violation. Final-best selection should use a tuple rank key `(invalid_flag, acc_violation, stab_violation, normalized_cost, …)`, not raw PPO reward. Reward v3 uses metric1 + metric2 gates and includes metric1_std/metric2_std in the stability gate, but those metric-std channels must tolerate normal small-K MRPC probe quantization; historical K=5 evidence remains useful, while current four-GPU runs use K=4. Do not use a tiny `1e-3` metric-std floor: the 2026-05-20 reward-v3 run at commit `6f3d618` failed at 345 episodes with P1=0, invalid=0, loss-cap=0 solely because normal metric-std jitter dropped 58 otherwise healthy episodes into P2 and pushed rolling300 below 35. Current behavior keeps tiny metric std jitter in P3 via a `1e-2` floor while still treating materially large metric std as P2. Current sequential Stage-2 cost reward is budgeted adaptive scalar: only P3 candidates receive cost reward; P3 metric margin has a small separate budget so it cannot crowd out cost ranking; fusion gain and truncation/K gain receive clear interval-style boosts; truncation/K uses a coarse layer-equivalent average-K tier (`cost_k_step_size=1/12`) rather than the older single-slot `1/59` tier because the 2026-05-23 fast-reward offline sweep showed `1/59` made about 27.5% of real P3 candidates saturate the cost clip too early, while `1/12` kept saturation near 9%; total bits is a separately clipped weak linear tie-breaker that should remain smaller than one fusion/K tier step. The bounded `terminal_cost_score` remains PPO shaping, but `terminal_cost_rank_score` is P3-only and unbounded, with fusion/truncation/bits components; best-action selection, top candidates, candidate-store ranking, and promotion/frontier seeds should use hard priority first and then this unbounded P3 rank. P1/P2 keep `terminal_cost_rank_score=0`, so cost still cannot compensate for accuracy or stability failure. `ParetoCostArchive` can still record frontier diagnostics and exploration stats, but Pareto event shaping is not the default PPO scalar reward.
8. **Invalid-chain pruning is layered, not a shape change.** Sequential rollout still keeps the full action vector and policy/critic head shapes. Before RL starts, `StaticInvalidLevelMask` performs a baseline-prefix, one-slot-at-a-time `Rescale_optimizer` feasibility scan and hides locally invalid `(layer, block, slot, level)` values in `action_level_mask`. This follows the COINN-style pre-search pruning idea and is deliberately more aggressive than runtime masking: some combinations may be discarded even if another prefix could have made them valid. The scan only calls `evaluate_step`; it commits baseline prefix actions only before the terminal step, so it does not run the terminal model-forward probe. Exact invalid step-action tuples still go into `ForbiddenActionMask`; repeated runtime invalid evidence goes into `EmpiricalInvalidLevelMask`. All three preserve the static baseline and current base/frontier proposal levels. Use diagnostics fields (`samples_rejected_by_mask`, `samples_rejected_by_optimizer`, `steps_fallen_back_to_baseline`, `forbidden_mask_total`, `static_invalid_level_disabled`, `static_invalid_level_applied`, `empirical_invalid_level_disabled`, `rejection_optimizer_wall_seconds`) to judge whether pruning improves runtime. Current 60k watchdog policy: post-anchor P1+P2 is a hard failure only above 30% after at least 100 post-anchor samples; sparse P1/P2 is warning-level noise, while invalid steps, loss-cap bursts, non-finite PPO, and broken four-GPU reward probes remain hard failures.
...
**2026-06-10 — episode-parallel mode SUPERSEDES the K-split scheme for fusion
runs (ADR-009).** `--stage2-rl-devices 0,1,…,N-1` (mirrors `--stage1-rl-devices`;
mutually exclusive with `--blb-v3-reward-devices`) runs N workers that each
collect COMPLETE episodes — GTrXL rollout on a per-window policy replica,
per-step replan, and the K reward trials run SERIALLY on the worker's own
GPU — assigned as balanced contiguous chunks of GLOBAL episode indices and
reassembled in global order for one PPO learner (deterministic pre-update
reseed). Key facts: (1) **K is a fixed hyperparameter (5), decoupled from the
GPU count** — the old K=NGPU convention itself made results depend on the
server's card count; (2) **all randomness is keyed by (run_seed, global
episode, step/trial/attempt)** via `blb_stage2_rl/seed_utils.py`, including the
probe noise (`function_handler.reseed_noise_rng_for_device`; the legacy noise
was os.urandom-seeded TRUE-RANDOM — never reproducible — and the K-split
path's `torch.manual_seed` calls never touched it) and the noisy preflight
(`PREFLIGHT_EPISODE=-1`); (3) same seed + same GPU arch → byte-identical
results for ANY device count, validated by diffing per-window
`[stage2-rollout] … rollout_sig=` lines between `--stage2-rl-devices 0` and
all-devices runs; (4) the K-split measurement: 5 GPUs gave only ~2.9× (probe
split 4.97× but rollout/install/bookkeeping serial), episode-parallel targets
~4.5–4.8× (residual = the serial PPO update, ~5 s per 60-episode window);
(5) scope is fusion-count mode only (all-valid map, no cross-episode mask /
frontier state); per-slot mode keeps the legacy serial loop + K-split, and
...
- **Block 1** — post-FFN / GELU output / Wffn2 / LayerNorm mean-variance head. Slots: GELU output fresh, Wffn2 weight encode, mean/variance scalar encodes, several rescales, block-end K. At layer 0 only the block-end K is effective: the cfg is installed with `noise_enabled=False`, so no Gaussian/rotation noise is added and K still executes on variance before rsqrt.
...
* `blb_stage2_training_curve.{png,npz}` — **Stage-1-aligned multi-panel** (2026-06-13): Reward / Loss / metric1 / metric2 / fusion_count / avg_K, each `raw + Moving Avg + Baseline` (mirrors Stage-1's `ppo_training_curve.png` style). NPZ carries every per-episode series. Older runs were a cruder 3-panel (reward-raw / flat-best / ppo-loss).
* `blb_stage2_entropy_curve.png` — **(2026-06-13)** standalone policy-entropy curve (raw + Moving Avg, x = "Episode (at PPO update)"), mirrors Stage-1's `ppo_entropy_curve.png`. Source: `ppo_updates.jsonl` entropy.
* `blb_stage2_search_log.txt` — **(2026-06-13)** local-optimum / health detection report in Stage-1's `pruning_search_log.txt` format (signals: entropy_collapsed / reward_plateau / best_stuck / action_diversity_collapsed + priority histogram). Generated by `rl_local_optimum.detect_rl_local_optimum` (torch-free, shared with Stage-1).
...
* `../best_policy/{blb_stage2_policy.pt, constraint_metadata.json}` — **(2026-06-13)** best checkpoint + constraints in the combo working dir, mirrors Stage-1's `best_policy/`. On decoupled completion the **sequential** runner now also archives `…/stage2/record/{combo N date}/` (`final_config.json` + `final_eval.json` + `metadata.json` + `report.md` + curves), writes a combo-level `metadata.json`, and marks the working dir `COMPLETED` — exactly like Stage-1. This was previously dead code reachable only by the non-default single-shot path (`runner.py` early-returns into `run_sequential_via_runner`), so finished Stage-2 runs used to lack the whole archive. Regenerate/backfill any run's curves + entropy + detection report offline (torch-free) with `python scripts/blb_regen_stage2_outputs.py <progress_dir>`.
...
- Block 1 → `block1_<dataset>` (SF/fusion replan skipped for layer 0; K-only
  model cfg still installed)
...
The historical training path treated every `env.step(action_vec)` as a horizon-1 episode that decided all 577 dims at once. The retired per-block sequential path decomposed that vector into optimizer-oriented steps:

```
horizon = 3 + (L-1) * 4    # 47 for L=12
order   = (L0,B2) -> (L0,B4) -> (L0,B5)
        -> (L1,B1) -> (L1,B2) -> (L1,B4) -> (L1,B5) -> ...
```

`first_input fresh` is folded into step 0. Do not hard-code old per-step slot counts here; use `step_schedule_max_dim(...)` or the live `seq_env.max_step_dim`. This legacy schedule omits layer-0 Block1 because it has no SF/RO step. The canonical layerwise policy is authoritative and does select/install layer-0 Block1 K.
...
### Fusion-count action (opt-in, 2026-06-03 — ADR-008, server-validated)

Replaces "decide every effective SF slot per block" with "decide `(fusion_option, K)` per block". `fusion_option` is resolved to the block's full SF vector via an **offline fusion-count map**; everything downstream (`action_vector_to_cfgs → bridge → replan → reward`) is unchanged. Enable with `--blb-v3-fusion-count-action 1` (default off; mutually exclusive with `--blb-v3-substage-mode`). When off, the per-slot path is bit-for-bit unchanged.

- **Offline map** (`blb_stage2_rl/fusion_maps/<profile>/<graph_key>.json`, in git; built by `scripts/blb_build_fusion_count_map.py` on the server, torch-free-ish, ~minutes). Per block-type (6 built since 2026-06-06: block1/2/4 + block5_n1/n2/n4; Block3 fusion/SF is baseline-owned and therefore has no map, but its layerwise K is active and installed; **block5_n0 / GELU degree 0 / ReLU disabled** — Stage-1 stopped sampling it so Stage-2 rejects a degree-0 layer at `baseline_bootstrap` (`ALLOWED_GELU_DEGREES=(1,2,4)`); the committed `block5_n0.json` + decode/handler are kept dormant for re-enable): enumerate effective chain slots (**rescale slots enumerate SF values only, never index-0/None — RL does not drop a must-exist op, mental-model item 2**), real `replan`, group by realized `fusion_count`, keep the **min post-override installed-variance** set (pluggable `NoiseOrder` in `fusion_count_map.py`; default summed installed variance). Ordering forces `option 0 == all-max baseline`. Builder uses the runner's *calibrated* `max_sfs` (not generic `load_max_sfs`). Pure grouping core (`fusion_enum.group_min_noise_options`) is torch-free + unit-tested; enumeration is server-only (`prepare_block_type_context`/`enumerate_shard`). mrpc result: block1/block4 = 1 option (genuinely fusion-degenerate → RL tunes only K there), the other 5 = 2 options (`fusion=[0,1]`, fusion=1 lowers source/encode SF to save bits).
- **Runtime** (`action_space.fusion_step_schedule` / `expand_fusion_step_action`; `sequential_env` fusion branch; `sequential_runner` `fusion_mode`): same GTrXL policy instantiated with `max_step_dim=2` (slot0=fusion option, slot1=K), `max_num_levels=max(6, max_options)`. Per step the env expands `(option,K)` → block SF vec → existing splice/replan/override/reward. Per-slot invalid masks + guarded-radius2 stay disabled (map is all-valid; no SF-locality), but a **block-granularity safe-neighbor curriculum is ON by default** (2026-06-05; `blb_stage2_rl/fusion_curriculum.py`, toggle `--blb-v3-fusion-neighbor-curriculum 0/1`). This **reverses** the original "fusion disables safe-neighbor" decision: unrestricted 47-block sampling from the first post-anchor episode compounds (even ~86% option0/block) into ~6–10 fusions/episode → accuracy collapse → flat P1 → no PPO gradient (the 2026-06-04 600-ep smoke went 511/600 P1, `loss_mean=100`, no recovery). The curriculum instead pins most blocks to baseline `(option 0, baseline K)` and lets only a **growing random subset** deviate within a **widening radius**; by the end of the ramp (`FUSION_NEIGHBOR_RAMP_FRACTION=0.5` × total episodes, override via `fusion_neighbor_ramp_episodes`) it **dissolves to the unrestricted open mask** — every block mutable, full radius — so it is a pure warmup that **never permanently masks any config** (full action space stays reachable; unit-locked in `tests/test_blb_fusion_curriculum.py::FullSpaceReachabilityTest`). The OFF setting reproduces the pre-2026-06-05 unrestricted behaviour for the A/B control. Anchor/warmstart/entropy/KL-LR kept. **2026-06-10 6k-episode 5-GPU A/B verdict: curriculum ON wins — judge by SEARCH PROGRESS, not tail mean reward.** ON's top candidates all landed at the END of training (best P3 40.62@ep5315, tail fusion 15.5, still improving); OFF found nothing after ep≈1000 (best 40.16@ep855) and parked at baseline (tail fusion 0.00) — its better tail mean (38.91 vs 38.82) is the safety of exploration collapse, not progress (ON's P3 episodes average ≈1.2 HIGHER; the mean is dragged by ON's 2.7% tail P1, which hard-priority best-selection never picks). `scripts/blb_fusion_ab_compare.py` now reports P2 (the "missing" 11% — P2 was always firing; it was just not displayed) and leads the both-healthy verdict with best-P3 + episode-of-best + tail-fusion (locked by `tests/test_blb_fusion_ab_verdict.py`). Forced anchor forces `(option 0, baseline-K idx)`; warmstart prior is K-slot-only since ADR-011 (`preferred=[-1, baseline_K_idx]`, gain `FUSION_WARMSTART_BIAS_GAIN=2.5` vs per-slot 1.2 — the old `[0, K]` fusion-slot prior never decayed to zero and helped cause the 60k fusion=0 collapse). Checkpoint variant suffixed `_fusioncount_v1` so a per-slot ckpt in the same dir is rejected cleanly on resume.
- **Reward (2026-06-03 redesign; spec `docs/superpowers/specs/2026-06-03-stage2-fusion-count-reward-design.md`):** keeps the hard-priority tier backbone (`accuracy > stability >> cost`; P1≈[-5,0] / P2≈[15,25] / P3≈[40,45]) but the **P3 cost** is a per-block-type weighted fusion+truncation saving — `reward.FUSION_COST_W={1:80,2:150,4:130,5:40}` + `TRUNC_COST_W=50`, normalized `/MAX_ACTUAL` (4630 for mrpc-47) × `p3_cost_budget`. Pure helper `blb_stage2_rl/fusion_cost.py::compute_fusion_cost_saving` (torch-free, from the per-block `(option,K)` choices `sequential_env` accumulates) feeds `compute_reward(external_cost_score=, external_cost_rank=)` (P3-only; bypasses the aggregate fusion/K/bits path). **`total_bits` is removed from the reward scalar** (still in diagnostics). block1/block4 fusion weights are inert (those maps are fusion-degenerate → K-only) and excluded from `MAX_ACTUAL`. Stability is the existing P2 std gate over a **K=4 cross-GPU probe** per episode (baseline preflight too); fusion mode warns on `num_trials_per_step<2` and force-disables fast-reward online-K=1. Cost is **terminal P3-gated** (never on P1/P2 — item 7). The reward change does not alter the policy shape, so it stays checkpoint-compatible (no `SEQ_RL_VARIANT` bump). Tests: `tests/test_blb_fusion_reward.py`.
  **2026-06-11 ADR-011 amendment (fusion-seeking fix):** the first 60k converged to fusion=0 while offline group eval proved block2/block5 fusion metric-free (one_hot_block2 loss 0.3837@fusion12 ≤ baseline 0.3846; block2+block5 fusion=24 ≈ no-op; only 12-layer block4 fusion is harmful, loss 0.62 → caught by P1). Root causes: shared normalization let the K pot (47×50) dilute one block5 fusion to +0.029 and let K-only consume the whole cost reward (1.71 ≈ 24-fusion 1.66 → fusion redundant); P3 margin favors the zero-noise baseline; the fusion-slot warmstart prior never decayed to zero; 15% P1/P2 failure tax under the 0.5% stability gate during the only window fusion was sampled. Fixes: **(a) split budget** — `ext_score = fusion_norm × (p3_cost_budget × FUSION_COST_BUDGET_FRACTION=2/3) + trunc_norm × (×1/3)`, components normalized over their OWN maxima (`FusionCostResult.fusion_norm/trunc_norm`; one block2 flip +0.117, block2+block5 full +1.78 ≫ margin cap 0.5, K-only ≤1.5); **(b) fusion option slot gets NO warmstart prior** (`preferred=[-1, K]`; `-1`=skip in `apply_preferred_per_step_bias`); **(c) scheduled forced-fusion probes** (`fusion_curriculum.fusion_probe_target_block`, flag `--blb-v3-fusion-probe-interval`, default 200, 0=off): every interval one episode forces fusion=1 on a rotating block type (b2→b5→b4) at baseline K via the forced/anchor mechanism under the open mask — pure function of absolute episode index (deterministic, 1==N preserved, ~0.5% overhead; the block4 probe intentionally yields truthful P1 evidence); **(d)** `RewardWeights.stab_tolerance` (v3 m1/m2/loss std channels) is now wired from `--stage2-stability-tolerance` (was silently stuck at 0.5). Rerun tolerances (user spec, run flags not code defaults): stability 5.0 (500%), metric 0.005 (0.5%). Reward values are NOT comparable across this ADR. Note: in the current step-1×15 maps block4 IS fusable, so its 130 weight counts (block1 stays inert).
  **2026-06-12 ADR-012 amendment (navigable boundary + exploration floor — the 2nd 60k STILL hit fusion=0 and the forensics overturned the diagnosis):** the split budget WORKS (on-policy fusion episodes beat ±40-ep no-fusion peers by +0.05..+0.19 in every phase); the real killer was the **P1 cliff tax** — 1226/1327 P1s were borderline fusion episodes (m1∈[0.833,0.858], zero catastrophic; no-fusion P1s: ONE in 60k): fusion+deep-K sits at the tolerance boundary and the 256-sample probe's quantization noise (σ≈0.0018) stochastically drops it below threshold for the full −46 gulf → expected fusion advantage −3.8 → 14606 on-policy attempts died by ep 16.5k; ADR-011's probes were neutralized (forced baseline-K cancelled the fusion gain: b2 net +0.07, b5 net **−0.86**, b4 100/100 catastrophic P1) and the policy froze from update ~700 (entropy=0.000, clip=0.000, 18k wasted episodes). Fixes: **(a) graded near-miss tier** (`RewardWeights.near_miss_tier_cap/floor/band` = 35/15/1.0): non-invalid metric fails within 1×|baseline−thr| earn tier 35−20×deficit instead of the cliff (typical borderline P1: −7→~26.6); priority stays 1 (selection/rank-key bit-identical) and cap 35 < P3 floor 40 so item 7 holds in the scalar too; beyond-band/catastrophic keep the cliff; **(b) borderline retest** (`BLBStage2EnvConfig.borderline_retest_enabled`, 2×trials on a golden-ratio-salted episode-keyed probe seed, retest verdict replaces the first; false-fail ≈quadratically down, 1==N preserved, `borderline_retest` info field); **(c) probe v2** — probes run the normal curriculum-masked sampling path and only force the TARGET block's option to 1 (mask-injected level + post-sample override + evaluate_action re-eval), K and other blocks follow the current policy (standing advantage ≈ +1.4 instead of ≈0); rotation `(2,5)` — **block4 dropped**; **(d) ε exploration floor** (`--blb-v3-fusion-exploration-epsilon`, option slot 0.05 / K slot 0.02): per-slot categorical becomes the mixture (1−ε)π+ε·Uniform(masked support) in BOTH sampling and PPO replay (ε=0 reproduces the old distribution exactly; `set_slot_exploration_epsilon`) so fusion choice can never become deterministic again. Reward scale near the boundary changes again across this ADR; judge runs by fusion adoption + `near_miss` flags, not raw P1 counts (sparse near-miss = exploration cost). Expected optimum: b2+b5 fused (24) + K-depth-that-fits, reward ≈41+ vs no-fusion cap ≈39.5.
  **2026-06-13 ADR-013 amendment (Stage-1-style log-barrier — the 3rd 60k flipped to a HOT collapse):** with ADR-012 active the run did the OPPOSITE of the first two — fusion marched MONOTONICALLY 1.4→35, metric1 0.866→0.690, and the back 30k episodes (50% of the run) froze flat at reward −6.95 (every episode catastrophic P1, zero gradient). Root cause: ADR-012's graded near-miss tier made overshooting the boundary nearly free (−7 cliff → 15–35), so with the monotone fusion cost incentive (esp. block4 @130, the most accuracy-toxic) and a tiny ~1.3% accuracy budget consumed JOINTLY by fusion+deep-K, the policy had a downhill ramp UP the fusion axis with no restoring force, then fell off into a flat no-gradient basin. (Hard-priority selection still rescued an excellent ep20880 config: fusion 22, P3, reward 40.8 > no-fusion 39.5 — the search works; it just doesn't STABILIZE.) Fix (user-chosen direction; cost weights kept 80:150:130:40 so the barrier is the SOLE restoring force): **port Stage-1's two-piece log-barrier** (`layer_importance_evaluator.py:log_barrier_reward`) onto the accuracy margin via `reward.accuracy_margin_barrier(mu)` where `mu`=worst signed margin in |baseline−thr| units. It REPLACES the near-miss tier (P1) AND the linear P3 metric-margin: **satisfied** (0≤mu<MARGIN_REF) = `SAT·(log(mu+eps)−log(MARGIN_REF+eps))` ≤0 with slope→∞ as mu→0 ⇒ `cost(fusion)+barrier(margin)` has an interior peak at POSITIVE margin (no overshoot); mu≥MARGIN_REF ⇒ 0 (cost decides); **violated** (mu<0) = `b0−VIO·(−mu)` continuous+LINEAR (no flat plateau over collapse depth) ⇒ recovery gradient (the missing piece). Defaults: `acc_barrier_enabled=True, sat_scale=0.5, margin_ref=0.25 (≈1.8 probe-σ — THE aggressiveness knob, sweep {0.15,0.25,0.35}), vio_scale=0.30, floor=−10`. `acc_barrier_enabled=False` ⇒ ADR-012 path (kept for A/B + NearMissGradedTierTest). Invariants: priority/rank-key/selection bit-identical (barrier rewrites only the PPO scalar), violated barrier < P3 floor 40 + P1 gets no cost ⇒ item 7 holds, pure fn of metrics ⇒ 1==N unaffected; invalid keeps legacy invalid_term. Retained from ADR-012: borderline-retest (denoises the margin the barrier reads), ε floor, policy-K probes. Added: per-block-type `fusion_count_b2/b4/b5` in episodes.jsonl; server-side collapse watchdog (sustained P3≈0 → kill PID; best already checkpointed). Reward NOT comparable across this ADR; judge by a non-collapsing curve + fusion stable at a positive-margin level (not 0, not 35) + P3>0 + best ≥ no-fusion cap. The stable optimum likely adopts FEWER than the knife-edge 22 fusions (leaves ~1.8σ headroom by design — reaching 22 stably is a probe-resolution problem, the documented next lever). Locks: `tests/test_blb_log_barrier_reward.py`.
  **2026-06-14 ADR-014 amendment (structural anti-runaway + collapse-debug落盘 — the 4th 60k STILL hot-collapsed):** ADR-013's log-barrier alone did NOT hold — fusion ran away 8→35 again, watchdog-killed at 40320 (artifacts `stage2_grid_gate_60k_20260613_175503`). Forensics: the best point's `terminal_metric1_std≈0.0155` EXCEEDS the feasibility margin (~0.013), ~8.6× the 0.0018 baseline σ ADR-013 calibrated MARGIN_REF=0.25 against → the barrier headroom is noise-drowned, can't form a measurable restoring attractor, while the LINEAR fusion cost (`fusion_norm`×budget) is a deterministic monotone incentive that wins. Fix (user-chosen; **cost weights AND probe size/K kept** — must work with existing noise): **(a) concave/saturating fusion cost** — `fusion_cost.saturate_fusion(x,tau)=(1−exp(−x/tau))/(1−exp(−1/tau))` applied to `fusion_norm`; the env scales `fusion_norm_saturated` (raw kept for diagnostics); `FUSION_SATURATION_TAU=0.15` ⇒ ~80% of fusion reward by `fusion_norm≈0.23` (≈fusion 8, below the noisy boundary 10-13), steep initial slope (no cold collapse) + flat tail (no hot runaway); `tau≤0`=identity=ADR-013. **(b)** `DEFAULT_ACC_BARRIER_MARGIN_REF` 0.25→0.5. Together `cost(fusion)+barrier(margin)` peaks at a moderate POSITIVE margin — max fusion never optimal (locked `tests/test_blb_fusion_saturation.py`). Stable optimum adopts FEWER fusions than the knife-edge 27 by design (probe-resolvable headroom) — a stable WIN over no-fusion, not a regression. item7/1==N/priority/rank/selection bit-identical; reward NOT comparable across this ADR. **(c) collapse-debug instrumentation (the "can only guess" fix):** the ADR-013 barrier/margin was computed but NEVER persisted — now `worst_signed_margin`(mu)/`acc_barrier_sat`/`acc_barrier_vio`/`near_miss`/`margin_m1`/`margin_m2`/`fusion_norm_raw`/`fusion_norm_saturated`/`fusion_count_b2/b4/b5` land in `episodes.jsonl`; `blb_stage2_health.log` (rolling600 P1/P2/P3+fusion+per-block+margin, was server-bash only) + `blb_stage2_diagnostics_curve.png` (priority/fusion/margin/reward-components/noise-vs-margin) + collapse attribution (`rl_local_optimum.attribute_collapse` → HOT/COLD+onset, in `blb_stage2_search_log.txt`) are emitted; `scripts/blb_regen_stage2_outputs.py` replays it all offline from any run's `episodes.jsonl`. Knobs if it settles too low/high: `FUSION_SATURATION_TAU`, `acc_barrier_margin_ref`. See `docs/adr/ADR-014-structural-anti-runaway-fusion-cost.md`.
  **2026-06-14 ADR-015 amendment (continuous bounded reward + strict stability — REBUILD, not a 5th patch):** after the 4th 60k the user studied the curves + told us the reward amplitude is too large, the exploration/initial-policy are wrong, and Stage-2 (unlike Stage-1) doesn't produce a config that STRICTLY satisfies accuracy+stability ("没看到稳定性约束"), and to **port the original Stage-2 (`noise_rl_module_v2._compute_terminal_reward_mc`) + Stage-1 (`layer_importance_evaluator._compute_final_reward`) designs**. Study verdict: Stage-1 = continuous log-barrier(loss,m1,m2)+cost, /20, clip[-5,5] + strict `_candidate_meets_constraints` selection + cosine entropy; original Stage-2 = adds a strict std stability gate (1.2×). The current fusion Stage-2's ±40 amplitude came from the tier 0/+20/+40 jumps at the boundary, and the std gate's threshold was mis-implemented as `(1+tol)+floor` fractional slack with no strict feasibility selection wiring it into the reported best. Fix (user Q1=continuous, Q2=real-std-multiplier-gate): **(1) `reward.py` `reward_design="continuous"` (default)** — `stage1_log_barrier(margin)` (satisfied `SAT·log(margin+eps)`, violated `-VIO·exp(-margin·STEEP)`) over the performance margins (m1/m2 means higher-better allow ≤`limit_tol` DROP + **loss_mean lower-better allow ≤`limit_tol` RISE** — 2026-06-15 user "loss 也是", folded into `metric_ok`/priority, mirroring Stage-1's loss/m1/m2; each margin in its OWN direction so a higher-better mean is never ×std-tol — the "1.2↔0.8" trap) + the signed std margins (m1_std/m2_std/loss_std lower-better × `stab_tol`), + P3-gated cost_frac, `clip(raw/NORM + W_cost·cost_frac, -5, +5)`; loss_mean gating is continuous-only (tiered rollback keeps m1/m2-only gate bit-identical, `loss_threshold` plumbed env→reward); NO tiers → bounded + continuous across the boundary; hard priority/item7 via WEIGHTING (violated barrier ≫ cost) + strict selection. Constants mirror Stage-1 (VIO=10/STEEP=20/SAT=0.5/NORM=20; W_acc=W_stab=1, W_cost=4). `reward_design="tiered"` = ADR-014 rollback. **(2) std-multiplier stability gate** (2026-06-15 user-corrected): `thr = max(baseline.X_std × tol, floor)` — `tol` is a MULTIPLIER on noisy-baseline std, NOT fractional slack (was `(1+tol)+floor`). Code default 1.2 (= original Stage-2's 1.2×); the run uses LENIENT `--stage2-stability-tolerance 5.0` (= 5×) because the user's std requirement isn't strict — 500% is a REAL gate, NOT vacuous (std > 5× baseline still → P2 → no cost reward). The PRIMARY anti-runaway is (1)'s continuous bounded reward + the 0.5% accuracy gate + (3)'s strict selection, not the std gate. ADR-014 saturation retired (`fusion_saturation_tau` default 0). **(3) strict feasibility selection** (`sequential_runner`, port of Stage-1 `_select_stage1_reward_best_config`): reported best must be priority==3 (strict acc+stab) else fall back to baseline. **(4) Stage-1 cosine entropy** (`_resolve_cosine_ent_coef_schedule`: 0.05→0.001, 25% plateau, lower-bound 0.012) replaces the low→high anchor+ramp; under continuous the ADR-011/012 patches are gated OFF (anchor / warmstart prior / fusion probes / ε floor / curriculum). KEEP: fusion-count action+maps, determinism+episode-parallel, ADR-014 debug落盘. Invariants: item7/1==N/checkpoint-compat. Honest expectation: with the lenient 5× std gate fusion is mainly bounded by the 0.5% accuracy gate + strict feasibility selection; fusion may stay modest — the intended trade (user prefers strict satisfaction over maximal fusion). Locks: `tests/test_blb_continuous_reward.py`. See `docs/adr/ADR-015-continuous-bounded-reward-strict-stability.md`.
  **2026-06-16 ADR-016 amendment (headroom cost + recovery gradient — fix the 5th-60k fusion runaway):** ADR-015 ran as the 5th 60k (`77ffdc0`; 1==N gate PASSED — determinism held) but STILL hot-collapsed (watchdog killed at 42180, "12 windows P3<2%"). Code+data forensics (`stage2_grid_gate_60k_20260615_170844`) found three compounding mechanisms: **(1) cost lure dominates the feasible region** — `scalar = barrier_raw/NORM + W_cost·cost_frac`, `CONT_W_COST=4.0 ≫ CONT_W_ACC=1.0`, satisfied barrier `0.5·log(margin)/20 ≈ −0.02` ≪ cost (real ep4259 P3: cost=2.43 ≫ barrier/NORM≈−0.02) → policy pushes fusion to the accuracy knife-edge; **(2) knife-edge** — cost is P3-gated → cliffs 2.4→0 at the boundary; **(3) flat violated plateau** — `−10·exp(−margin·20)` saturates at margin<−0.25 → clip −5, so a MILD violation (m1=0.84) and a CATASTROPHIC one (m1=0.63) earned the IDENTICAL −5 (zero recovery gradient → an overshoot is permanent). Health log: healthy until ep~7000 (P3≈92%, fusion≈18) then one-way ratchet 18→24→30→36, froze at max (−6.941). The saved best was healthy (fusion≈19, P3) — only the trajectory collapsed. Fix (user chose "精修连续 reward"): **(A) headroom-coupled cost** (`_continuous_reward`): `headroom = clip(min(acc∪std margins)/CONT_COST_HEADROOM_MARGIN_REF, 0, 1)`, `scalar = barrier_raw/NORM + W_cost·cost_frac·headroom` — cost fades smoothly to 0 as the worst margin→0 (no cliff) and is 0 in violation (item7) → restoring force → stable interior optimum at a SAFE margin (not the boundary). **(B) linear recovery gradient** (`stage1_log_barrier` violated branch): `CONT_BARRIER_VIOLATION_SLOPE·m` (linear, was `−VIO·exp(−m·STEEP)`) → a milder violation always scores strictly higher → policy climbs back out of P1; bounded by the clip only at extreme depth. **Constants calibrated by an OFFLINE reward-landscape replay over the 5th run's real 42180 episodes** (defaults MARGIN_REF=1.0, SLOPE=4.0, W_cost=4.0 give interior peak@fusion≈18, monotone decline past it f18 +0.73→f24 −0.49→f36 −2.81, recovery gradient P1 mild −0.36≫deep −2.88) — validate-the-landscape-before-60k is itself the process fix vs the prior 5 ship→60k→collapse loops. Invariants: item7/1==N/bounded[−5,5]/checkpoint-compat. Reward NOT comparable across ADR-016. Locks: `tests/test_blb_continuous_reward.py::ADR016LandscapeTest`. See `docs/adr/ADR-016-headroom-cost-and-recovery-gradient.md`.
- **Block3 baseline-owned SF plus policy-owned K** (2026-07-21; supersedes the 2026-06-03 full-removal workaround): the regenerated archives now contain a valid `block3_exp_n6`. `load_static_skeletons_baseline` reads Block3 for all layers and produces the exact calibrated `ss_action_vec`; `BLBStage2LayerwiseEnv` resets from that vector, leaves every Block3 SF/fusion slot unchanged, and writes only the selected K. Terminal cost evaluation includes Block3 in `build_optimizer_requests`; RO replans only the fixed SF chain and optimizer write-back preserves K. `BLBNoiseRLBridge.apply` installs the final per-layer Block3 cfg so `_make_block3_approximation_exponential` executes real post-polynomial truncation. RO valid-block count remains 59 because layer-0 Block1 has no SF request; policy/model K count is 60.
