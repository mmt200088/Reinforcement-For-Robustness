...
3. **Actions are integer INDICES, not scaling factors.** Policy outputs `a_j ∈ {0,…,m_j-1}` per slot; a decode rule (`sf_from(idx, max_sf, levels)` for SF slots; the `K_LEVELS` table for K slots) maps each index to the actual `scaling_factor` or truncation `k`. **2026-06-04 decode — uniform 10-level HYBRID sweep for all SF kinds (F/W/M/S/R)** (a uniform step-2 / floor-12 grid was tried 2026-06-10 and REVERTED the same day per user decision — every slot keeps the fixed 10-level hybrid sweep): `LEVELS_F=W=MS=R=10`; the slot's top 5 levels step by 2 (coarse, near baseline), bottom 5 step by 1 (fine), reaching baseline-14. e.g. baseline 30 → `30,28,26,24,22,20,19,18,17,16`. **The anchor `max_sf` is the slot's BASELINE SF** (calibrated `MaxSFsTable` from static_skeletons), **not** the noise-table max (46): you sweep *down from the baseline SF*. `_snap_to_table` floors any decoded SF below the noise table (min SF=10) — a low-baseline-SF slot gets duplicate SF=10 levels; the fusion builder skips those pre-enumeration via `action_space.distinct_sf_level_indices` (result-equivalent acceleration: a duplicate-value level decodes to the identical cfg; the lowest index per value is kept = the lex-min representative the post-eval signature dedup would keep). Rescale (R) keeps idx0=None. **Changing this grid invalidates ALL committed fusion maps (they store action indices) — rebuild via SERVER_COMMAND before any fusion run.** **Noise variance uses the N=16384 column for every slot** (`_block_default_N` forced to 16384; N=8192 retired). Baseline (max idx → baseline SF) is unchanged → `option0==baseline` holds. **Fusion-map builder pinning uses the `(fusion_count, total_bits)` criterion (NOT fusion-only).** The 2026-06-04 "relax to fusion_count only" change was a bug, reverted: committed ground-truth maps show every block's fusion>0 option is reached by lowering 2–4 non-rescale ENCODE SFs *jointly* while all rescales stay at baseline (e.g. block2 fc=1 = `inv_std_fresh 28→20, gamma 20→16, wk 22→16`); no single encode moves fusion alone, so a fusion-only solo-probe pins them all and the map collapses to `fusion={0}`. `total_bits` here is a build-time over-enumeration PROXY (it is NOT in the reward) that keeps every fusion-driving encode in the joint cartesian enumeration. 2026-06-10 build acceleration (result-equivalent): the builder enumerates only distinct-value levels per slot (skips snap-floored duplicates) and decodes only the target `(layer, block)` via `action_vector_to_cfgs(..., only=...)` (per-(L,B) decode independence makes this bit-identical; the full 12-layer decode, not the sub-ms replan, dominated per-combo cost — also speeds the 47-step training rollout). The deep 10-level build can still be large; `--max-enum-combos N` is the budget guard for an over-budget block-type (over-budget → degeneracy probe; refuses to shortcut if anything fuses; block4 full enum was ~7×10⁸ combos / 12.7h pre-acceleration). Always log both `action_index` AND `decoded_value`. K decoding is **non-monotonic** in some checkpoints — find all-max via the largest `k` value, not the largest index.
