...
3. **Actions are integer INDICES, not scaling factors.** Policy outputs `a_j ∈ {0,…,m_j-1}` per slot; a decode rule (`sf_from(idx, max_sf, levels)` for SF slots; the `K_LEVELS` table for K slots) maps each index to the actual `scaling_factor` or truncation `k`. **2026-06-10 decode — UNIFORM step-2 sweep with a hard floor (supersedes the 2026-06-04 hybrid 2/1 sweep):** `LEVELS_F=W=MS=R=10`; level value = baseline − 2·dist (e.g. baseline 30 → `30,28,26,24,22,20,18,16,14,12`), and **no slot may decode below `MIN_SF_FLOOR=12`**. **The anchor `max_sf` is the slot's BASELINE SF** (calibrated `MaxSFsTable` from static_skeletons), **not** the noise-table max (46): you sweep *down from the baseline SF*. SELECTABLE levels are the strict arithmetic-sequence members ≥ 12 (`action_space.distinct_sf_level_indices`): an odd baseline stops above the floor (27 → `27,25,…,13`, NO pseudo-12 level), a low baseline keeps fewer than 10 levels (16 → `16,14,12`), and a baseline below the floor keeps exactly its one frozen baseline level — generic for any dataset/baseline. `sf_from` clamps defensively at `min(12, baseline)` for indices that should never be selected. Rescale (R) keeps idx0=None. **Changing this grid invalidates ALL committed fusion maps (they store action indices) — rebuild via SERVER_COMMAND before any fusion run.** **Noise variance uses the N=16384 column for every slot** (`_block_default_N` forced to 16384; N=8192 retired). Baseline (max idx → baseline SF) is unchanged → `option0==baseline` holds. **Fusion-map builder pinning uses the `(fusion_count, total_bits)` criterion (NOT fusion-only).** The 2026-06-04 "relax to fusion_count only" change was a bug, reverted: committed ground-truth maps show every block's fusion>0 option is reached by lowering 2–4 non-rescale ENCODE SFs *jointly* while all rescales stay at baseline (e.g. block2 fc=1 = `inv_std_fresh 28→20, gamma 20→16, wk 22→16`); no single encode moves fusion alone, so a fusion-only solo-probe pins them all and the map collapses to `fusion={0}`. `total_bits` here is a build-time over-enumeration PROXY (it is NOT in the reward) that keeps every fusion-driving encode in the joint cartesian enumeration. The build enumerates only the selectable (distinct, ≥12) levels per slot and decodes only the target `(layer, block)` via `action_vector_to_cfgs(..., only=...)` (2026-06-10 — per-(L,B) decode independence makes this bit-identical; the full 12-layer decode, not the sub-ms replan, dominated per-combo cost), which shrinks block4 from ~7×10⁸ hybrid combos to roughly 10⁷-scale; `--max-enum-combos N` remains a budget guard (over-budget → degeneracy probe; refuses to shortcut if anything fuses). Always log both `action_index` AND `decoded_value`. K decoding is **non-monotonic** in some checkpoints — find all-max via the largest `k` value, not the largest index.
