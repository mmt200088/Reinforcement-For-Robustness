...
- **Runtime** (`action_space.fusion_step_schedule` / `expand_fusion_step_action`; `sequential_env` fusion branch; `sequential_runner` `fusion_mode`): same GTrXL policy instantiated with `max_step_dim=2` (slot0=fusion option, slot1=K), `max_num_levels=max(6, max_options)`. Per step the env expands `(option,K)` → block SF vec → existing splice/replan/override/reward. Open per-slot mask only (map is all-valid); **invalid masks + safe-neighbor + guarded-radius2 are disabled**; anchor/warmstart/entropy/KL-LR kept. Forced anchor forces `(option 0, baseline-K idx)`; warmstart `preferred=[0, baseline_K_idx]` with a stronger gain (`FUSION_WARMSTART_BIAS_GAIN=2.5` vs per-slot 1.2). Checkpoint variant suffixed `_fusioncount_v1` so a per-slot ckpt in the same dir is rejected cleanly on resume.
