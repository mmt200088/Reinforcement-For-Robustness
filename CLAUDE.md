...
- **block 3 fully removed from the baseline** (2026-06-03, prerequisite for this): `_resolve_stage2_fixed_stage1_config` pins `FIXED_SOFTMAX_DEGREE=6`, but the regenerated `static_skeletons_mrpc.json` has `block3_exp_n6=success:false`. Since block 3 is frozen + already out of the schedule + never installed by the bridge, it is now also dropped from `baseline_bootstrap.load_static_skeletons_baseline` (block loop `(1,2,3,4,5)→(1,2,4,5)`) and `action_space.build_optimizer_requests` (skip `block_idx==3`). Baseline + action cost stay consistent (both exclude block 3). `valid_block_count` is now `47` (4×12−1), not 59. This unblocked ALL stage-2 runs.
