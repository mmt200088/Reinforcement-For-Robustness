...
def main() -> None:
    ...
    manifest = {
        ...
        "description": "Complete BERT-large SST2 Stage-2 RL graceful-stop snapshot at episode 35280.",
    ...
    stop_lines = [
        f"captured_at={dt.datetime.now().astimezone().isoformat()}",
        "graceful_stop_request=STOP_RL marker; exact marker timestamp was not separately persisted",
        "graceful_stop_boundary=35280",
        "graceful_stop_updates=294",
        run_text(["pgrep", "-af", "bert-large.*sst2"]),
    ...
    readme = f"""# BERT-large SST2 Stage-2 RL graceful-stop archive
