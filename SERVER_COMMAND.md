...
- **任务**：在 Claude 修复 action → 真实模型计算配置链路后，重新运行 BLB Stage-2 sequential RL，并先加入 guarded radius2 探索优化。
...
- **本次目标**：
  ...
  - 先启动 fresh 1000 episode dual-GPU smoke run，确认新 action → cfg 链路和 guarded radius2 诊断正常。
...
- **本次实验假设**：上一版 Pareto-only 10k 全程 P3、无 invalid、无稳定性失败，但后期 frontier 扩展少，dominated/duplicate 样本占比高。历史 raw radius2 曾在 `radius=2, mutations=8/9` 区间触发 P1 cluster，因此仍保持默认 raw `NEIGHBOR_MAX_RADIUS=1`，只在 frontier 停滞且最近健康时打开受控 radius2：
