...
> **本文件使用 `$...$` / `$$...$$` 数学语法，可在 GitHub、Cursor、VS Code、Typora、Obsidian 等支持 KaTeX/MathJax 的 markdown 渲染器中直接渲染。**
...
$$
\|\delta_C(x)\|_\infty \;\le\; \beta_C(x). \tag{1}
$$
...
\mathcal{S}_C
\;=\;
\Big\{\,(x, y) \in \mathcal{D}\;:\; \gamma(x, y) \;>\; 2 \beta_C(x) \,\Big\}, \tag{2}
$$
...
\mathrm{CertAcc}(C)
\;=\;
\frac{|\mathcal{S}_C|}{|\mathcal{D}|}. \tag{3}
$$
...
$$
\mathrm{Acc}_{\mathrm{enc}}(C) \;\ge\; \mathrm{CertAcc}(C). \tag{4}
$$

**(ii)** *对任意两个配置 $A$ 与 $B$，若*

$$
\beta_A(x) \;\le\; \beta_B(x), \quad \forall (x, y) \in \mathcal{D}, \tag{5}
$$
...
\qquad
\mathrm{CertAcc}(A) \;\ge\; \mathrm{CertAcc}(B). \tag{6}
$$
...
\qquad
\epsilon_\ell^{\mathrm{logit}}(C) := \frac{\epsilon_\ell^{\mathrm{ct}}(C)}{\Delta_\ell(C)}, \tag{7}
$$
...
\;+\;
\underbrace{P_\ell(x; C)}_{\text{近似算子}}. \tag{8}
$$
...
\qquad \delta_0 \in \{10^{-6}, 10^{-9}, \ldots\}. \tag{9}
$$
...
\qquad \forall \ell \in \{1, \ldots, L\}, \tag{10}
$$

*则 (5) 自动成立，故*

$$
\mathrm{CertAcc}(A) \;\ge\; \mathrm{CertAcc}(B). \tag{11}
$$

**证明**：由 (7) 直接代入；逐项 $\le$ 之和仍 $\le$。 $\blacksquare$
...
\qquad \forall x \in \mathcal{D}, \tag{12}
$$
...
\;\ge\;
\mathrm{CertAcc}_{\mathrm{sim}}(C^\star), \tag{13}
$$

*即 simulation-based CertAcc 是真实密文 accuracy 的下界。*

**证明**：由 (12) 与定理 1(i) 直接得到。 $\blacksquare$
