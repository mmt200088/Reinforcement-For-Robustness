...
def split_replan_payload(payload: Any) -> Tuple[Optional[List[int]], Dict[str, DeltaValue]]:
    ...
    * ``{"node_name": delta, ...}`` for backward-compatible bare deltas.
    ...
    if keys & {"t_new", "delta_overrides", "propagation_deltas"}:
        ...
        return t_new, _normalize_delta_overrides(deltas)

    return None, _normalize_delta_overrides(payload)
...
class ReplanSession:
    ...
    def __call__(self, graph_key: str, payload: Any) -> Dict[str, Any]:
        ...
        t_new, delta_overrides = split_replan_payload(payload)
        return self.replan(
            graph_key,
            t_new=t_new,
...
def graph_key_for_stage1(
    *,
    ...
    if block == 5:
        ...
        return f"block5_n{int(degrees[layer])}"
