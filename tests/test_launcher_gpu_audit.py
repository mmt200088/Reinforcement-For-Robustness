...
class LauncherGpuAuditTest(unittest.TestCase):
    ...
    def test_stage1_warns_when_stage1_devices_use_only_part_of_visible_gpus(self):
        ...
        self.assertEqual(len(warnings), 1)
        self.assertIn("Stage-1 RL uses 2 of 4 visible GPUs", warnings[0])
        self.assertIn("--stage1-rl-devices 0,1,2,3", warnings[0])
    ...
    def test_stage2_warns_when_parallel_device_flags_disagree(self):
        audit = _load_audit_module()
        ...
        warnings = audit.audit_launch(
            ...
            cuda_visible_devices="0,1,2,3",
            stage1_rl_devices="",
            stage2_rl_devices="0,1,2,3",
            blb_v3_reward_devices="0,1",
            stage2_k_trials=4,
        )

        self.assertTrue(any("Stage-2 device flags disagree" in item for item in warnings))
        self.assertTrue(any("reward probes use 2 of 4 visible GPUs" in item for item in warnings))
    ...
    def test_strict_cli_returns_zero_when_no_warning_exists(self):
        audit = _load_audit_module()

        rc = audit.main([
            "--search-algorithm", "rl",
            "--run-mode", "stage2-only",
            "--stage2-rl-variant", "blb_v3",
            "--cuda-visible-devices", "0,1",
            "--stage1-rl-devices", "",
            "--stage2-rl-devices", "0,1",
            "--blb-v3-reward-devices", "",
            "--stage2-k-trials", "4",
            "--strict",
        ])
