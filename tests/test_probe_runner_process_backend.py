...
class ProbeBackendSelectionTest(unittest.TestCase):
    ...
    def test_single_device_replicas_use_threads_scopes_and_streams(self):
        class _FakeModel:
        ...
        primary_model = _FakeModel("primary")
        primary_batch = object()
        replica_index = 0

        def clone_model(_model):
            nonlocal replica_index
            replica_index += 1
            return _FakeModel(f"replica-{replica_index}")

        with mock.patch.object(
                _probe_runner, "BLBNoiseRLBridge", return_value=object(),
                ), mock.patch.object(
                    _probe_runner, "ReversibleLayerHandler", return_value=object(),
                ), mock.patch.object(
                    _probe_runner.copy, "deepcopy", side_effect=clone_model,
                ), mock.patch.object(
                    _probe_runner, "_move_probe_batch_to_device",
                    ...
                    _probe_runner.torch.cuda, "device",
                ), mock.patch.object(
                    _probe_runner.torch.cuda, "Stream",
                    side_effect=lambda: _FakeStream(),
                ), mock.patch.object(
                    _probe_runner.mp, "get_context",
                ) as get_context:
            owner = _probe_runner.build_probe_runner(
                ...
                is_regression=False,
                device_ids=[0],
                metric_profile="single-gpu",
                workers_per_device=3,
            )

        self.assertEqual(owner.backend, "thread")
        self.assertEqual(owner.num_workers, 3)
        self.assertEqual([str(worker.device) for worker in owner.workers], [
            "cuda:0", "cuda:0", "cuda:0",
        ])
        self.assertEqual(
            len({worker.probe_noise_scope for worker in owner.workers}),
            3,
        )
        self.assertTrue(
            all(worker.probe_cuda_stream is not None for worker in owner.workers)
        )
