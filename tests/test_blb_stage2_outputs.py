...
class RegeneratorEndToEndTest(unittest.TestCase):
    ...
    def test_jsonl_readers_use_shared_iter_jsonl(self):
        ...
        entropy_rows = [
            {"entropy": 1.5, "completed_episodes": 120},
        ]
        ...
        def fake_iter_jsonl(path, **kwargs):
            ...
            if os.path.basename(path) == "episodes.jsonl":
                return iter(episode_rows)
            if os.path.basename(path) == "ppo_updates.jsonl":
                return iter(entropy_rows)
