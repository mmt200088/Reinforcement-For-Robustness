...
class OrdinaryRunnerApiTest(unittest.TestCase):
    ...
    def test_completed_ga_extension_allows_only_the_full_run_contract_change(self):
        invocation_matches = getattr(
            search_baseline_runner,
            "_stage2_ga_full_run_invocation_extension_matches",
            None,
        )
        contract_matches = getattr(
            search_baseline_runner,
            "_stage2_ga_full_run_extension_contract_matches",
            None,
        )
        self.assertIsNotNone(invocation_matches)
        ...
        requested_invocation = json.loads(json.dumps(legacy_invocation))
        requested_invocation["scientific_parameters"][
        ...
        requested_algorithm = json.loads(json.dumps(legacy_algorithm))
        requested_algorithm["search_config"]["evaluation_budget"] = 11_464
        requested_algorithm["search_config"].update({
            "ga_generations": 200,
        ...
        legacy_contract = {
            ...
            "requested_manifest": {
                ...
                "algorithm_contract": legacy_algorithm,
                "algorithm_contract_hash": stable_json_hash(legacy_algorithm),
                "strict_identity_context_hash": "a" * 64,
            },
        }
        requested_contract = json.loads(json.dumps(legacy_contract))
        requested_contract["evaluation_budget"] = 11_464
        requested_contract["search_config"].update({
            "ga_generations": 200,
            "ga_maximum_evaluations": 11_464,
            "ga_stop_on_no_improvement": False,
            "ga_require_full_generations": True,
        })
        requested_contract["requested_manifest"][
            "stage2_invocation"
        ] = requested_invocation
        requested_contract["requested_manifest"][
            "algorithm_contract"
        ...
        self.assertTrue(contract_matches(
            legacy_contract,
            requested_contract,
        ))
        changed_contract = json.loads(json.dumps(requested_contract))
        changed_contract["seed"] = 43
        self.assertFalse(contract_matches(
            legacy_contract,
            changed_contract,
        ))


class OrdinaryStrictPayloadValidationTest(unittest.TestCase):
...
class OrdinaryRunnerPersistenceTest(unittest.TestCase):
    ...
    def test_completed_legacy_ga_extends_online_search_and_reruns_strict(self):
        base_result = _result()

        def ga_result(termination_reason):
            return SearchResult(
                algorithm="coinn_ga",
                best=base_result.best,
        ...
        def strict_validator(result):
            strict_calls.append(result.termination_reason)
            return _strict_payload(result)

        def legacy_search(
                _backend, _space, evaluator, _config, *, preload=(),
        ):
            self.assertFalse(preload)
            for item in base_result.observations:
                evaluator.on_evaluation(item.as_dict())
            return ga_result("ga_no_incumbent_improvement")

        resumed_preload_counts = []

        def extended_search(
                _backend, _space, _evaluator, _config, *, preload=(),
        ):
            resumed_preload_counts.append(len(tuple(preload)))
            return ga_result("generation_limit")

        with tempfile.TemporaryDirectory() as tmpdir:
            current_kwargs = _run_kwargs(
                tmpdir,
                backend="coinn_ga",
            ...
            with (
                ...
                first = search_baseline_runner.run_layerwise_search_baseline(
                    **current_kwargs
                )
            ...
            legacy_manifest["evaluation_budget"] = 45_664
            legacy_manifest["search_config"].update({
                "ga_generations": 800,
                "ga_maximum_evaluations": 45_664,
            })
            legacy_manifest["search_config"].pop(
                "ga_stop_on_no_improvement", None,
            )
            legacy_manifest["search_config"].pop(
                "ga_require_full_generations", None,
            )
            legacy_resume = legacy_manifest["resume_contract"]
            legacy_resume["evaluation_budget"] = 45_664
            legacy_resume["search_config"].update({
                "ga_generations": 800,
                "ga_maximum_evaluations": 45_664,
            })
            legacy_resume["search_config"].pop(
                "ga_stop_on_no_improvement", None,
            )
            legacy_resume["search_config"].pop(
                "ga_require_full_generations", None,
            ...
            with open(manifest_path, "w", encoding="utf-8") as handle:
                json.dump(legacy_manifest, handle)
            with (
                mock.patch.object(
                    search_baseline_runner,
                    "run_search",
                    side_effect=extended_search,
                ),
                mock.patch.object(
                    search_baseline_runner,
                    "_validate_ga_completion_proof",
        ...
        self.assertEqual(resumed_preload_counts, [5])
        self.assertEqual(
            strict_calls,
            ["ga_no_incumbent_improvement", "generation_limit"],
        )
        self.assertEqual(completed["manifest"]["preloaded_observation_count"], 5)
        self.assertEqual(
            completed["manifest"]["extension_from_completed_run"][
                "termination_reason"
            ],
            "ga_no_incumbent_improvement",
        )
        self.assertEqual(completed["manifest"]["status"], "complete_strict_feasible")
