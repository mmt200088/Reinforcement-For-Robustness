...
| Launcher gates | `9b7777a` | `experiments/server_command_runs/phase0_preflight_stream_9b7777a_20260704_140740/` | Stream Phase-0 preflight report files and entrypoint Markdown directly through the line writer instead of joining full output lists or rendering one full Markdown string before `Path.write_text()`. |
...
| Stage-1 diagnostics | `841d48e` | `experiments/server_command_runs/stage1_parallel_md_stream_841d48e_20260704_160500/` | Stream Stage-1 parallel Markdown reports through the CLI writer instead of rendering the full report string before `Path.write_text()`. |
...
| Paean final eval | `6d43e0d` | `experiments/server_command_runs/fusion_rlpath_html_stream_6d43e0d_20260704_132900/` | Stream fusion-count RL install-path comparison HTML reports directly through the CLI writer while preserving the existing `_render_html()` string helper for compatibility. |
...
| Stage-2 scheduling gate | `28ab0c0` | `experiments/server_command_runs/stage2_layerwise_ab_contract_28ab0c0_20260713/` | Replace the incompatible legacy episode-parallel A/B command with the production layerwise reward-device K-split contract, exact robust-run parameters, local CUDA-id remapping, and a no-GPU effective-command preflight. |
...
| Rescale/fusion maps | `6137460` | `experiments/server_command_runs/fusion_report_defaults_6137460_20260704_125319/` | Reuse fixed fusion-count map report GELU/Softmax default JSON strings instead of dumping the same defaults whenever the report parser is built. |
| Rescale/fusion maps | `c0d2a92` | `experiments/server_command_runs/fusion_map_report_html_stream_c0d2a92_20260704_134100/` | Stream fusion-count map HTML reports directly through the CLI writer while preserving the existing `_render_html()` string helper for compatibility. |
...
The new shared command builder uses logical `--blb-v3-reward-devices` ids,
batch 64, rollout 120, all-4 fixed config, and the complete robust constraint
...
Progress 2026-07-10: server cProfile showed MRPC session loading was already
only about `0.0029s`, while two graph-state restores per full replan spent
`0.234s` of a profiled 10,000-call run and dispatched 280,000 scalar
`deepcopy()` calls. Source commit `2907e63` now snapshots/restores the graph's
`int` / `Optional[int]` delta fields by direct assignment. On the same server,
200,000 restores improved from `0.616172s` to `0.169611s` (`3.63x`), and
50,000 full block4 replans with compact output improved from `2.050284s` to
`1.753686s` (`1.169x`) with an identical semantic signature. Direct RED/GREEN
...
Progress 2026-07-11: fusion templates already emit exact built-in
`str -> int/'x2'` delta dictionaries, but the session interface still parsed
and copied them for every combination. Source commit `8f5792a` returns only
that exact normalized representation by reference; all coercible or custom
inputs retain the original parser. Eleven-repeat hot-loop A/B improved 100,000
combinations from median `1.302531s` to `1.257134s` (`1.0361x`) with identical
result SHA256. The short three-repeat 20-worker block1 gate improved median
wall from `11.48s` to `11.41s` and reduced median user CPU by `1.74s`; all maps
...
Progress 2026-07-11: the compact fusion-map caller never consumes the
`applied_delta_overrides` echo dictionary, although the generic replan core
still allocated and filled it for every combination. Source commit `805e0ed`
lets only `ReplanSession.replan_compact()` skip that record; lookup, mutation,
and validation remain unchanged, and full replan diagnostics retain the
original default. Eleven-repeat hot-loop A/B improved 100,000 combinations
from median `1.197590s` to `1.179304s` (`1.0155x`) with identical result
SHA256. The short three-repeat 20-worker block1 builder gate was wall-neutral
(`11.05s` versus `11.09s`) while median user CPU fell by `0.46s`; all maps
...
Progress 2026-07-11: the generic replan core scanned `q_initial` once through
`any(...)` for non-positive values and again through a list comprehension for
over-`q_max` stages. Source commit `3e99adc` classifies both bounds in one
loop, preserves immediate non-positive precedence, and still records every
1-indexed over-limit stage when all drops are positive. Eleven-repeat hot-loop
A/B improved 100,000 combinations from median `1.107091s` to `1.027356s`
(`1.0776x`) with identical result SHA256. Three 20-worker full block1 builds
improved median wall from `10.84s` to `10.63s` (`1.0198x`), reduced median
