...
- [x] **Step 1: Write failing tests for the action space and six constraints**
...
- [x] **Step 3: Implement immutable actions, observations, normalized margins, and deterministic ranking**
...
- [x] **Step 1: Add failing deterministic optimizer tests**
...
- [x] **Step 2: Implement greedy search**
...
- [x] **Step 3: Implement constrained SMAC-style BO-RF**
...
- [x] **Step 4: Implement COINN-style GA**
...
- [x] **Step 1: Add failing adapter tests with a recording layerwise environment**
...
- [x] **Step 2: Implement the evaluator adapter and persistence**
...
- [x] **Step 3: Add optional strict final certification**
...
- [x] **Step 1: Add failing flag/config propagation tests**
...
- [x] **Step 2: Wire explicit search-backend and baseline-search settings**
...
- [x] **Step 3: Dispatch the layerwise branch**
...
- [x] **Step 4: Run wiring and regression tests**
...
- [x] **Step 3: Synchronize an isolated server source tree**
...
- [x] **Step 4: Run real-model smoke tests**

Run each backend with a tiny evaluation budget through the cached BERT-large
RTE Stage-2 environment. BERT-base MRPC was unavailable in the server's
offline cache, and physical GPUs 1 and 4 were unhealthy, so the smoke runs
shared healthy GPUs with the active PPO job. Require at least one real model
forward, verified replan installation before forward, complete loss/m1/m2 mean
and standard-deviation fields, exact action/K/fusion output, and a clean process
exit. Smoke mode may skip expensive A/B/C final certification, but it must label
that omission and must not export a standard scientific-best action.

- [x] **Step 5: Save smoke evidence and report**
