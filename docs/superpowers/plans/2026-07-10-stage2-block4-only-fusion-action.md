...
- [x] **Step 2: Run the tests on the server and verify RED**
...
- [x] **Step 3: Implement the minimal schedule mapping**
...
- [x] **Step 4: Run the schedule tests and verify GREEN**

Run the same unittest command. Expected: all `FusionScheduleTest` tests pass.

- [x] **Step 5: Commit the action-domain change**
...
- [x] **Step 2: Run focused tests on the server and verify RED**
...
- [x] **Step 3: Apply the canonical resolution once per step**
...
- [x] **Step 4: Run focused tests and the existing fusion contract suite**
...
- [x] **Step 5: Commit the canonical-path integration**
...
- [x] **Step 2: Verify RED on the server**
...
- [x] **Step 3: Implement the torch-free auditor**
...
- [x] **Step 1: Push the verified source branch and create an exact server snapshot**
...
- [x] **Step 2: Generate pair-specific fixed-action configs**
...
- [x] **Step 3: Run both GELU pairs concurrently on separate GPUs**
...
- [x] **Step 5: Render one comparison report**
...
- [x] **Step 1: Build into profile-local staging directories**
...
- [x] **Step 2: Audit before promotion**
...
- [x] **Step 3: Promote only clean profiles**
...
- [x] **Step 4: Pull compact artifacts and commit locally**
...
- [x] **Step 1: Run fresh server verification**
...
- [x] **Step 2: Perform local static verification only**
...
- Completed on the server and merged with the concurrent runtime-optimization
  work at source commit `d95e93483bbb9da91e3bc204ea079a8047b3296e`.
- The five non-MRPC profiles had byte-identical normalized Block2 contexts and
  templates, so one full 211,891,680-combination RTE Block2 enumeration was
  reused with recorded provenance. Their Block5 contexts were also identical
  to the verified MRPC Block5 contexts. Every target profile was independently
  re-boosted and passed golden replan, K-independence, and runtime-install
  verification; profile-specific Block4 maps were retained.
- All six profiles' required Block2/4/5 graphs have fusion counts exactly
  `[0, 1]`; no `fusion_count > 1` anomaly exists.
- Only MRPC currently has its full Stage-2 map bundle. The other five profiles
  were completed for the requested Block2/4/5 scope and still lack
  `block1_<profile>`; no unrequested Block1 map was synthesized.
