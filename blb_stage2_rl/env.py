...
class BLBStage2Env:
    ...
    def _compute_terminal_reward(
            ...
            *,
            ...
            info: Dict[str, Any],
            external_resource_objective: Optional[Mapping[str, Any]] = None,
            constraint_assessment_override: Optional[Any] = None,
            ) -> RewardBreakdown:
        assessment = None
        if (
            ...
            if not any_invalid:
                ...
                if constraint_assessment_override is None:
                    ...
                    assessment = constraint_assessment_override
                    info["statistical_assessment_provenance"] = (
    ...
    def _finish_prepared_terminal_probe(
        ...
        breakdown = self._compute_terminal_reward(
            ...
            constraint_assessment_override=constraint_assessment_override,
            info=info,
        )
    ...
    def evaluate_prepared_terminal_batch_protected_k1(
            self,
            prepared_list: Sequence[Mapping[str, Any]],
            *,
            absolute_episodes: Sequence[int],
            force_k5_mask: Sequence[bool],
            guard_sigma: float = 4.0,
            audit_fraction: float = 0.02,
            audit_seed: int = 0,
            validation_required: bool = False,
            ) -> List[Tuple[np.ndarray, float, bool, Dict[str, Any]]]:
        """Run trial 0 first and preserve exact K=5 for every protected action."""
        from .protected_k1 import (
            decide_protected_k1,
        ...
        episodes = [int(value) for value in absolute_episodes]
        forced = [bool(value) for value in force_k5_mask]
        if not (
                len(prepared_items) == len(episodes) == len(forced)
        ):
            raise ValueError(
                "prepared terminals, absolute episodes, and force_k5_mask "
                "must have equal lengths"
            )
        if not prepared_items:
            return []
        if self.probe_runner is None or not hasattr(
                self.probe_runner, "run_action_trial_groups_at_indices",
        ):
            raise RuntimeError(
                "protected K=1 requires exact trial-index probe scheduling"
            )
        reference = getattr(self, "statistical_reference", None)
        if reference is None:
            raise RuntimeError(
                "protected K=1 requires a statistical baseline reference"
            )

        forward_indices = [
            index for index, item in enumerate(prepared_items)
            if bool(item.get("requires_forward", True))
        ]
        if any(
                prepared_items[index].get("probe_base_seed") is None
                for index in forward_indices
        ):
            raise RuntimeError(
                "protected K=1 requires deterministic per-episode probe seeds"
            )
        first_results_by_item: Dict[
            int, Tuple[float, float, float]
        ] = {}
        first_diag_obj = None
        first_diag: Dict[str, Any] = {}
        if forward_indices:
            first_grouped = self.probe_runner.run_action_trial_groups_at_indices(
            ...
            if len(first_grouped) != len(forward_indices):
                raise RuntimeError(
                    "protected K=1 first stage returned the wrong action count"
                )
            for item_index, trial_group in zip(
                    forward_indices, first_grouped,
            ):
                if len(trial_group) != 1:
                    raise RuntimeError(
                        "protected K=1 first stage must return one trial"
                    )
                first_results_by_item[item_index] = tuple(trial_group[0])
            first_diag_obj = self.probe_runner.last_diagnostics
            first_diag = (
                diagnostics_payload(first_diag_obj)
                if first_diag_obj is not None else {}
            )
            self._probe_eval_counter += len(forward_indices)
            self._installed_config_fingerprint = None

        decisions: Dict[int, Any] = {}
        audited: Dict[int, bool] = {}
        continuation_indices: List[int] = []
        gate_probability = float(getattr(
            self, "statistical_gate_probability", 0.50,
        ))
        for item_index in forward_indices:
            item_base_seed = int(
                prepared_items[item_index]["probe_base_seed"]
            )
            first_metrics = self._metrics_from_trial_results(
                [first_results_by_item[item_index]],
                trial_seeds=[
                    derive_probe_trial_seed(item_base_seed, 0)
                ],
            )
            decision = decide_protected_k1(
                (
                    first_metrics.loss_mean,
                    first_metrics.metric1_mean,
                    first_metrics.metric2_mean,
                ),
                reference,
                guard_sigma=float(guard_sigma),
                gate_probability=gate_probability,
                force_k5=forced[item_index],
            )
            decisions[item_index] = decision
            audit = bool(
                decision.screened
                and should_audit_protected_k1(
                    int(audit_seed),
                    episodes[item_index],
                    float(audit_fraction),
                )
            )
            audited[item_index] = audit
            if not decision.screened or audit:
                continuation_indices.append(item_index)

        followup_by_item: Dict[
            int, List[Tuple[float, float, float]]
        ] = {}
        followup_diag_obj = None
        followup_diag: Dict[str, Any] = {}
        if continuation_indices:
            followup_grouped = (
                self.probe_runner.run_action_trial_groups_at_indices(
                    [
                        prepared_items[index]["decoded"]
            ...
            if len(followup_grouped) != len(continuation_indices):
                raise RuntimeError(
                    "protected K=1 follow-up returned the wrong action count"
                )
            for item_index, trial_group in zip(
                    continuation_indices, followup_grouped,
            ):
                if len(trial_group) != 4:
                    raise RuntimeError(
                        "protected K=1 follow-up must return trials 1..4"
                    )
                followup_by_item[item_index] = [
                    tuple(result) for result in trial_group
                ]
            followup_diag_obj = self.probe_runner.last_diagnostics
            followup_diag = (
                diagnostics_payload(followup_diag_obj)
                if followup_diag_obj is not None else {}
            )
            self._installed_config_fingerprint = None
        ...
        followup_local_index = {
            item_index: local_index
            for local_index, item_index in enumerate(continuation_indices)
        }

        def per_worker_indices_for_action(
                diag_obj: Any,
                action_index: Optional[int],
                ) -> List[List[int]]:
            if diag_obj is None or action_index is None:
                return []
            return [
                [
                    int(trial_index)
                    for observed_action, trial_index in tasks
                    if int(observed_action) == int(action_index)
                ]
        ...
        results: List[Tuple[np.ndarray, float, bool, Dict[str, Any]]] = []
        for item_index, item in enumerate(prepared_items):
            if item_index not in first_results_by_item:
                metrics = self._placeholder_metrics_for_invalid()
                runtime = self._finish_prepared_terminal_probe(
                    item,
                    metrics,
                    probe_diagnostics={
                        "protected_k1_enabled": True,
                        "protected_k1_trials_executed": 0,
                ...
                runtime[3].update({
                    ...
                    "protected_k1_trials_executed": 0,
                    "protected_k1_stability_measured": False,
                })
                results.append(runtime)
                continue

            decision = decisions[item_index]
            was_audited = audited[item_index]
            trial_results = [first_results_by_item[item_index]]
            trial_results.extend(followup_by_item.get(item_index, ()))
            expected_trial_count = (
                5 if (not decision.screened or was_audited) else 1
            )
            if len(trial_results) != expected_trial_count:
                raise RuntimeError(
                    "protected K=1 assembled an unexpected trial count"
                )
            base_seed = int(item["probe_base_seed"])
            trial_seeds = [
                derive_probe_trial_seed(base_seed, trial_index)
                for trial_index in range(expected_trial_count)
            ]
            metrics = self._metrics_from_trial_results(
                trial_results,
                trial_seeds=trial_seeds,
            )
            k1_only_reject = bool(decision.screened and not was_audited)
            first_group_wall = float(
                first_diag.get("wall_seconds", 0.0) or 0.0
            )
            followup_group_wall = float(
                followup_diag.get("wall_seconds", 0.0) or 0.0
            )
            first_worker_indices = per_worker_indices_for_action(
                first_diag_obj,
                first_local_index.get(item_index),
            )
            followup_worker_indices = per_worker_indices_for_action(
                followup_diag_obj,
                followup_local_index.get(item_index),
            )
            first_worker_seconds = [
                float(value) / max(1, len(forward_indices))
                for value in getattr(
                    first_diag_obj, "per_worker_seconds", (),
                )
            ]
            followup_worker_seconds = [
                float(value) / max(1, len(continuation_indices))
                for value in getattr(
                    followup_diag_obj, "per_worker_seconds", (),
                )
            ] if item_index in followup_by_item else []
            worker_count = max(
                len(first_worker_indices),
                len(followup_worker_indices),
                len(first_worker_seconds),
                len(followup_worker_seconds),
            )
            per_worker_indices = []
            per_worker_seconds = []
            for worker_index in range(worker_count):
                per_worker_indices.append(
                    (
                        first_worker_indices[worker_index]
                        if worker_index < len(first_worker_indices) else []
                    )
                    + (
                        followup_worker_indices[worker_index]
                        if worker_index < len(followup_worker_indices) else []
                    )
                )
                per_worker_seconds.append(
                    (
                        first_worker_seconds[worker_index]
                        if worker_index < len(first_worker_seconds) else 0.0
                    )
                    + (
                        followup_worker_seconds[worker_index]
                        if worker_index < len(followup_worker_seconds) else 0.0
                    )
                )
            per_worker_seeds = [
                [
            ...
            amortized_probe_wall = (
                first_group_wall / max(1, len(forward_indices))
                + (
                    followup_group_wall / max(
                        1, len(continuation_indices),
                    )
                    if item_index in followup_by_item else 0.0
                )
            )
            combined_speedup = (
                sum(per_worker_seconds) / amortized_probe_wall
                if amortized_probe_wall > 0.0 and per_worker_seconds
                else 1.0
            )
            per_action_diagnostics = {
                **(first_diag if k1_only_reject else followup_diag),
                "k": int(expected_trial_count),
                "action_count": 1,
                "trials_per_action": int(expected_trial_count),
                "multi_action": False,
                "wall_seconds": float(amortized_probe_wall),
                ...
                "speedup_vs_sequential": float(combined_speedup),
                "line": (
                    "[probe-runner] protected-k1 action="
                    f"{int(first_local_index[item_index])} "
                    f"k={int(expected_trial_count)} "
                    f"first_group_actions={len(forward_indices)} "
                    f"followup_group_actions={len(continuation_indices)} "
                    f"wall={float(amortized_probe_wall):.3f}s"
                ),
                "fast_reward_mode": True,
                "online_num_trials_per_step": 5,
                "terminal_eval_batch_size": int(len(forward_indices)),
                "probe_install_wall_seconds": float(amortized_probe_wall),
                "probe_install_skipped": False,
                "probe_clear_wall_seconds": 0.0,
                "probe_clear_skipped": True,
                "persistent_probe_install": True,
                "protected_k1_enabled": True,
                "protected_k1_screened": bool(decision.screened),
                "protected_k1_audited": bool(was_audited),
                "protected_k1_k1_only_reject": k1_only_reject,
                "protected_k1_reason": str(decision.reason),
                "protected_k1_violating_channels": list(
                    decision.violating_channels
                ),
                "protected_k1_worst_precision_z": float(
                    decision.worst_precision_z
                ),
                "protected_k1_guard_sigma": float(guard_sigma),
                "protected_k1_trials_executed": int(expected_trial_count),
                "protected_k1_stability_measured": (
                    expected_trial_count >= 2
                ),
                "protected_k1_first_group_wall_seconds": first_group_wall,
                "protected_k1_followup_group_wall_seconds": followup_group_wall,
                "protected_k1_first_group_action_count": len(forward_indices),
                "protected_k1_followup_group_action_count": len(
                    continuation_indices
                ),
                "validation_required": bool(validation_required),
            }
            self._last_probe_diagnostics = dict(per_action_diagnostics)
            runtime = self._finish_prepared_terminal_probe(
                item,
                metrics,
                probe_diagnostics=per_action_diagnostics,
                forward_ran=True,
                constraint_assessment_override=(
                    decision.assessment if k1_only_reject else None
                ),
            )
            runtime_info = runtime[3]
            breakdown = runtime_info.get("reward_breakdown")
            priority = int(
                breakdown.get("priority", 0)
                if isinstance(breakdown, Mapping)
                else getattr(breakdown, "priority", 0)
            )
            audit_precision_false_negative = bool(
                decision.screened and was_audited and priority >= 2
            )
            audit_p3_false_negative = bool(
                decision.screened and was_audited and priority == 3
            )
            telemetry = {
                "protected_k1_enabled": True,
                "protected_k1_screened": bool(decision.screened),
                "protected_k1_audited": bool(was_audited),
                "protected_k1_k1_only_reject": k1_only_reject,
                "protected_k1_audit_precision_false_negative": (
                    audit_precision_false_negative
                ),
                "protected_k1_audit_p3_false_negative": audit_p3_false_negative,
                "protected_k1_reason": str(decision.reason),
                "protected_k1_violating_channels": list(
                    decision.violating_channels
                ),
