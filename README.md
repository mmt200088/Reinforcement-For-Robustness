...
completion. A complete two-stage run writes `search_best_config.json` only when
strict validation selects a candidate. Comparator observations, histories,
checkpoints, and strict evidence remain under `stage1_comparator/<algorithm>/`
and `stage2/progress/search_<algorithm>/`. PPO checkpoints, candidate journals,
...
without `--fresh` to resume. A formal best-config JSON is not written when a
run is interrupted, fails, remains incomplete, or completes without a strict
selection.
